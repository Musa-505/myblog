...
if (isset($_POST['submit'])){
	...
	if ((empty($icon)) or (empty($post)) or (empty($title))){
		...
		exit();
	}else{$mysqli->query("INSERT INTO edu (icon, title, post) VALUES('$icon', '$title', '$post')") or die($mysqli->error);

	}
}
if (isset($_GET['delete'])){
	$id = $_GET['delete'];
	$mysqli->query("DELETE FROM edu WHERE id=$id") or die($mysqli->error());
	header("Location: ../page/home.php");
}
